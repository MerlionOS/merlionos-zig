...
comptime {
    if (hello_exec_prefix.len != 0x78) @compileError("hello_exec_prefix p_offset must stay at 0x78");
    if (user_programs.hello.len != 0x40) @compileError("hello_exec_prefix p_filesz/p_memsz must match user_programs.hello");
}

pub const hello_exec = hello_exec_prefix ++ user_programs.hello;
